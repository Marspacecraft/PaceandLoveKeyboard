...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    ...
    {
       	sg_MS_Count++;//5ms
		if(0 == (sg_MS_Count%2))//10ms
		{
			count = sg_MS_Count;
			count = sg_MS_Count/2;
			item.size = count;
			item.handler = CycleTimer_Task;
			item.event = EVENT_CYCLE_TIMER;
